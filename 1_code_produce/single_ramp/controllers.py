...
class PiAlineaController(BaseController):
    ...
    def execute_control(self, raw_state_list, is_training=False):
        ...
        action_ratio = max(self.min_ratio, min(self.max_ratio, raw_action_ratio))

        return action_ratio, raw_action_ratio, None, None, None, [False, None]
...
class RLController(BaseController):
    ...
    def execute_control(self, raw_state_list, is_training=False):
        ...
        lower_bound = calculate_lower_bound(prev_demand, curr_queue,
                                            RAMP_CAPACITY_VEH_S, SIM_STEPS_PER_CONTROL, W_MAX, ALPHA, R_MIN_RATIO)
        replaced = int(action_ratio < lower_bound)

        if self.use_replacement and replaced:
            action_ratio = min(1.0, max(action_ratio, lower_bound))

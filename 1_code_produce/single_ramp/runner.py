...
from config import ALPHA, MAX_TTS, AVG_TTS, W_MAX, RAMP_CAPACITY_VEH_S, SIM_STEPS_PER_CONTROL
...
def run_episode(env, controller, control_steps, sim_steps_per_control, is_training=False):
    ...
    history = {"green_times": [], "raw_actions": [], "queues": [], "downstream_speeds": [], "tts_total": 0, "lower_bound": []}
    num_replacements = 0
    ...
    for step in range(control_steps):
        ...
        if replaced:
            penalty = calculate_penalty(current_queue, raw_state[7], action_ratio,
                                    RAMP_CAPACITY_VEH_S, SIM_STEPS_PER_CONTROL, W_MAX, ALPHA)
        ...
        spillback = final_queue > ALPHA * 42.0
        ...
        if raw_action is not None:
            history["raw_actions"].append(raw_action.item())
        elif isinstance(log_prob, (int, float)):
            history["raw_actions"].append(float(log_prob))

...
SINGLE_RAMP_DIR = os.path.dirname(os.path.abspath(__file__))
CODE_DIR = os.path.dirname(SINGLE_RAMP_DIR)
...
def run_single_experiment(args):
    ...
    cmd = [sys.executable, os.path.join(SINGLE_RAMP_DIR, "train.py"),
           "--seed", str(seed), "--num_episodes", str(500)]
    if use_replacement:
        cmd.append("--use_replacement")

    # Run silently to avoid terminal spam, only print on completion
    subprocess.run(
        cmd, check=True, cwd=SINGLE_RAMP_DIR,
